...
track <-
  ...
  #geom_text(aes(x=31,y=-99,label="Texas")+
  # geom_text(aes(x = 3, y = 1, label ="Texas"))+
...
（Insert hurricane exposure package image）
```{r}
# function
...
(Insert hurricane exposure package image）-Rain
```{r}
hurr = "Humberto-2007"
...
(Insert hurricane exposure package image）-wind
```{r}
...
(Insert three time-series plots)
```{r}
hurr_Humberto2007 %<>% mutate(dist_42019 = NA, dist_42035 = NA, dist_srst2 = NA)
for(i in 1:  dim(hurr_Humberto2007)[1]){
  hurr_Humberto2007$dist_42019[i] <- distHaversine(c(hurr_Humberto2007$longitude[i], hurr_Humberto2007$latitude[i]), 
